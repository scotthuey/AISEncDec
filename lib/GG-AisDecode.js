...
function AisDecode (input, session) {
    ...
    // -- commented out other message types with differing lengths work as well.
    // Need to gauge impact and enhancements necessary for other messages i.e. !SAVDM, etc.
    ...
    // -- commented out other message types work as well. Need to gauge impact and enhancements necessary for other messages
    //if (nmea[0] !== '!AIVDM' && nmea[0] !== '!AIVDO') return;

    //if(nmea.length) return;
    ...
    if(message_count > 1) {
        ...
        if(message_id > 1) {
            if(nmea[0] !== session.formatter) {
                ...
                this.err = { "message": "AisDecode: Sentence does not match formatter of current session" };
                return;
            }
            ...
            if(session[message_id - 1] === undefined) {
                ...
                this.err = { "message": "AisDecode: Session is missing prior message part, cannot parse partial AIS message." };
                return;
            }

            if(session.sequence_id !== sequence_id) {
                console.log ("AisDecode: Session IDs do not match. Cannot recontruct AIS message.");
                this.err = { "message": "AisDecode: Session IDs do not match. Cannot recontruct AIS message." };
            }
        } else {
    ...
    // extract binary payload and other useful information from nmea paquet
    try {
    ...
    switch (this.aistype) {
        ...
        case 3: // class A position report
            ...
            this.navstatusname = NAV_STATUS[this.navstatus];
        ...
        case 19: // Extended class B position report 
            ...
            this.vesselType = VESSEL_TYPE[this.cargo];
...
//          3 = station compliant with future editions
        ...
        case 4: // base station
            this.class      = '-';
            this.shipname = "AIS BASE STATION";
