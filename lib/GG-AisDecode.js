...
function AisDecode (input, session) {
    ...
    if(message_count > 1) {
        ...
        if(message_id > 1) {
            if(nmea[0] !== session.formatter) {
                ...
                return { err: "AisDecode: Sentence does not match formatter of current session" };
            }
            ...
            if(session[message_id - 1] === undefined) {
                ...
                return { err: "AisDecode: Session is missing prior message part, cannot parse partial AIS message." };
            }

            if(session.sequence_id !== sequence_id) {
                console.log ("AisDecode: Session IDs do not match. Cannot recontruct AIS message.");
                return { err: "AisDecode: Session IDs do not match. Cannot recontruct AIS message." };
            }
        } else {
    ...
    if(message_count > 1) {
        ...
        if(message_id < message_count) return { partial: true };
